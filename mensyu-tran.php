...
.dark header { background: linear-gradient(135deg, #1a0a2e, #0d1a2e); }
.dark .input-section { background: rgba(15,15,40,0.9); border-color: rgba(255,255,255,.08); }
.dark .output-section { background: rgba(15,15,40,0.9); border-color: rgba(255,255,255,.08); }
.dark .output-content { background: rgba(10,10,30,0.8); border-color: rgba(255,255,255,.08); color: #e2e8f0; }
.dark textarea { background: rgba(10,10,30,0.8); border-color: rgba(255,255,255,.15); color: #e2e8f0; }
.dark .floating-original { background: rgba(15,15,40,0.95); border-color: rgba(124,58,237,.4); }
.dark .floating-content { color: #e2e8f0; }
.dark .floating-original-text { color: #e2e8f0; }
.dark .original-text { background: rgba(15,15,40,.8); color: #e2e8f0; }
.dark .translation-text { background: rgba(15,15,40,.7); color: #e2e8f0; }
.dark .character-breakdown { background: rgba(15,15,40,.7); color: #e2e8f0; }
.dark .copyright-footer { background: rgba(15,15,40,.9); color: #94a3b8; }
/* Micro-interactions */
...
<div class="container">
<header>
<a id="back-btn-link" href="./index.php" class="back-btn" aria-label="返回">
    <i class="fas fa-arrow-left"></i> 返回
</a>
<!-- Translation logo replaced with platform image -->
<div class="header-brand">
  <img src="https://i.ibb.co/gMQh9L2S/Edu-Genius-AI.png" alt="EduGenius AI Logo" aria-hidden="true" style="width:52px;height:52px;border-radius:14px;object-fit:cover;flex-shrink:0">
  <h1>文樞</h1>
</div>
<p class="header-subtitle">文言文翻譯工具</p>
</header>
...
<p>Copyright © 2026 Hugo Wong. All rights reserved</p>
</div>
</footer>
</div>
...
if (window.saveHistoryRecord) {
    window.saveHistoryRecord(text, translatedText);
}
} else {
...
<script type="module">
    ...
    window.saveHistoryRecord = async function(originalText, translatedText) {
        ...
        try {
            ...
            await fetch('./api/history.php', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        { role: 'user', content: originalText, timestamp: new Date().toISOString() },
                        { role: 'assistant', content: translatedText, timestamp: new Date().toISOString() }
                    ]
                })
            });
        } catch (e) {
